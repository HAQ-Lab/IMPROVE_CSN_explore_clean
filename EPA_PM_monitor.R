...
csn_site = read.csv("/Users/ztttttt/Documents/HEI PMF/R - original CSN/CSN metadata 157 sample merged sites 2010-20.csv")
csn_site = select(csn_site, Site, Country, State, County,  AQSCode, Latitude,  Longitude, StreetAddress)
...
csn_site$long_approx <- round(csn_site$Longitude, digits = 3)
csn_site$lat_approx <- round(csn_site$Latitude, digits = 3)

# Match the datasets based on the approximated values
matched_site <- merge(csn_site, aqs_site, by = c("long_approx", "lat_approx"))
